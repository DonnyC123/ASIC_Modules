...
#(
  ...
  localparam EXP_OVFL_IDX = EXP_IN_W + CARRY_W - 1;
  localparam EXP_SIGN_IDX = EXP_IN_W + SIGN_W + CARRY_W - 1;

  localparam NORMAL_FRAC_LSB_IDX = FULL_SUM_W - 1 - FRAC_IN_W;
  ...
  logic        [MANTISSA_IN_W-1:0] sum_frac_carry;
  ...
  always_comb begin
    ...
    if (sum_float_flags_i.c_dominates) begin
      ...
      sum_frac_raw = normalized_mantissa[FULL_SUM_W-1-:FRAC_IN_W];
    ...
    sum_frac_carry = sum_frac_raw + FRAC_IN_W'(round_mantissa);

    sum_rounded_exp_raw = (sum_frac_carry[MANTISSA_IN_W-1] && !sum_exp_ovfl) ? sum_exp + 1 : sum_exp;
    ...
    sum_frac_rounded = sum_frac_carry[FRAC_IN_W-1:0];

    float_sum_rounded.exp = sum_rounded_exp_raw[EXP_IN_W-1:0];
