...
#(
  ...
  logic [PRODUCT_MANTISSA_W-1:0] partial_products[NUM_WT_ROWS];
  logic [PRODUCT_MANTISSA_W-1:0] wt_sum;
  logic [PRODUCT_MANTISSA_W-1:0] wt_carry;
  ...
  always_comb begin
    for (int i = 0; i < MANTISSA_W; i++) begin
      partial_products[i] = norm_mant_b_i[i] ? (PRODUCT_MANTISSA_W'(norm_mant_a_i) << i) : '0;
    end
    ...
    partial_products[MANTISSA_W] = csa_c_i;
  end
  ...
  always_comb begin
    ...
    mantissa_sum_lower = PARTIAL_SUM_LOW_W'(wt_sum) + PARTIAL_SUM_LOW_W'(wt_carry);
